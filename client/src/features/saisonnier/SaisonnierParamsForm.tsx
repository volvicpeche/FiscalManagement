...
export function SaisonnierParamsForm() {
  const { params, updateParams } = useSaisonnierStore();

  const pctField = (label: string, key: keyof typeof params, step = 0.1) => (
    <div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Horizon (annees)</label>
          <input
            type="number"
            min={1}
            max={50}
        ...
        {pctField('Revalorisation CA (%)', 'rentGrowthRate')}
